...
try:
    file_name = name + '.kirisame.pyword.json'
    path = os.getcwd()
    a_path = os.path.join(path, "json/" + file_name)
...
def app():
    ...
    try:
        ...
        word_t = "未在互联网上查询到此内容。请检查网络是否成功连接以及您是否有拼写错误。您所输入的内容是：" + key_1;
